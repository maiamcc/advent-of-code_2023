...
func TestMinSet(t *testing.T) {
	// We know the parseGame fn works, so just parse from string b/c I don't wanna write out these game structs
	// also recall, we represent the min set as a round
	cases := map[string]round{ // map input to expected output
		"Game 1: 5 red":           {5, 0, 0},
		"Game 2: 5 green; 5 blue": {0, 5, 5},
		"Game 3: 1 blue, 2 green, 3 red; 3 blue, 2 green, 1 red; 1 blue, 3 green, 1 red": {3, 3, 3},
		"Game 4: 6 red; 9 blue, 5 red; 6 green":                                          {6, 6, 9},
		"Game 5: 1 red; 1 red, 2 blue; 1 red, 5 blue; 1000 red":                          {1000, 0, 5},
	}

	for input, expected := range cases {
		t.Run(input, func(t *testing.T) {
			g, err := parseGame(input)
			assert.Nil(t, err)
			actual := g.minSet()
			assert.Equal(t, expected, actual)
		})
